...
import { ClaimEmailScreen } from "./components/ClaimEmailScreen";
...
export type EnrollMode = "enroll" | "claim";

const BASE_STEPS: EnrollStepId[] = [
  "member-agreement",
  "telehealth-consent",
  "consents",
...
export function EnrollFlow({ mode = "enroll" }: { mode?: EnrollMode }) {
  const steps: EnrollStepId[] =
    mode === "claim" ? ["email", ...BASE_STEPS] : BASE_STEPS;

  const [state, setState] = useState<EnrollState>(initialState);
  const [history, setHistory] = useState<EnrollStepId[]>([steps[0]]);
  const [submitting, setSubmitting] = useState(false);
  const current = history[history.length - 1];

  // enroll mode: capture an optional ?email= from the pricing handoff.
  useEffect(() => {
    if (mode !== "enroll" || typeof window === "undefined") return;
